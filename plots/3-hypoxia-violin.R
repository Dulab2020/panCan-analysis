...
setwd('~/research/2020single/finalplot/plot3-hypoxia-M/')
...
for (i in names(gsvidx)){
  ...
  md <- subset(d,mtag=='M')
  hypm <- md@meta.data[,c('Hypoxia','cell_type')]
  ...
  p<-ggviolin(hypm,x='cell_type',y='Hypoxia',fill='cell_type')+NoLegend()+scale_fill_simpsons()+
