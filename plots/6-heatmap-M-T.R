...
setwd('~/research/2020single/finalplot/plot6-heatmap-M-T/')
...
strt<-function(x){
  if(is.na(x)){
    ...
    if (startsWith(x,'M'))
      return('M')
  else if(startsWith(x,'CD')|startsWith(x,'NK')){
    return('T')
  }else
    return('others')
}
...
for (i in 1:6){
  ...
  mye <- subset(d,mtag=='M')
  Idents(mye)<-'cell_type'
  tce <- subset(d,mtag=='T')
  Idents(tce)<-'cell_type'
  print('M')
  for (j in genes$M){
    if (j %in% rownames(mye$SCT@data)){
    }else{print(j)}
  }
  print('T')
  for (j in genes$T[1:13]){
    if (j %in% rownames(tce$SCT@data)){
    }else{print(j)}
  }  
  if(can=='OV'){
    gapm = c(7,12)
    avgm<-AverageExpression(mye,assays = 'SCT',slot = 'data',features = genes$M[-9])$SCT
  }else
  { gapm = c(7,13)
    avgm<-AverageExpression(mye,assays = 'SCT',slot = 'data',features = genes$M)$SCT}
  
  avgt<-AverageExpression(tce,assays = 'SCT',slot = 'data',features = genes$T[1:13])$SCT
  bk <- c(seq(-1.5,0,length.out = 50),seq(0.01,1.5,length.out = 50))
  #myeloid cells
  detach("package:pheatmap", unload = TRUE)
  library(pheatmap)
  mp <- pheatmap(avgm,scale = 'row',cluster_rows = F,gaps_row = gapm,angle_col = 45,legend = T,
                 treeheight_col = 8,cellwidth = 12,cellheight = 12,border_color = NA,
                 color = colorRampPalette(colors = c('navy','white','firebrick3'))(100),breaks = bk) 
  mp <- ggplotify::as.ggplot(mp)
  #tcells
  ...
  tp <- pheatmap(avgt,scale = 'row',cluster_rows = F,gaps_row = c(3,11),angle_col = 45,legend = F,
                 treeheight_col = 8,cellwidth = 12,cellheight = 12,border_color = NA,
                 color = colorRampPalette(colors = c('navy','white','firebrick3'))(100),breaks = bk)
  tp <- ggplotify::as.ggplot(tp)
  print(mp+tp)
